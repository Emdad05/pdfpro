        ...
        <div className="space-y-4">
          ...
          {processing && (
            <div className="glass-card p-4">
              <div className="flex justify-between text-sm mb-2"><span className="text-slate-300">Converting pages...</span><span className="text-accent-400">{progress}%</span></div>
              <div className="progress-track"><div className="progress-fill" style={{ width: `${progress}%` }} /></div>
            </div>
          )}
          {done ? (
            <div className="success-card"><div className="text-4xl mb-3">✅</div><p className="font-heading font-semibold text-emerald-300 text-lg mb-3">Images downloaded as ZIP!</p><button onClick={() => { setFile(null); setDone(false); }} className="btn-glass text-sm">Convert another</button></div>
          ) : (
            <div className="flex gap-3"><button onClick={convert} disabled={processing} className="btn-primary flex-1 justify-center">{processing ? 'Converting...' : 'Convert to JPG'}</button><button onClick={() => setFile(null)} className="btn-glass">Reset</button></div>
          )}
        </div>
      )}
    </ToolLayout>
