        ...
        <div className="space-y-4">
          ...
          <div className="glass-card p-5 space-y-4">
            <div>
              ...
              <input type="password" value={password} onChange={e => { setPassword(e.target.value); setError(''); }} className={`glass-input ${error ? 'border-red-500/50' : ''}`} placeholder="Enter PDF password" onKeyDown={e => e.key === 'Enter' && unlock()} />
          ...
          {done ? (
            <div className="success-card"><div className="text-4xl mb-3">✅</div><p className="font-heading font-semibold text-emerald-300 text-lg mb-3">PDF unlocked!</p><button onClick={() => { setFile(null); setDone(false); setPassword(''); }} className="btn-glass text-sm">Unlock another</button></div>
          ) : (
            <div className="flex gap-3"><button onClick={unlock} disabled={processing || !password} className="btn-primary flex-1 justify-center">{processing ? 'Unlocking...' : 'Unlock PDF'}</button><button onClick={() => setFile(null)} className="btn-glass">Reset</button></div>
          )}
        </div>
      )}
    </ToolLayout>
