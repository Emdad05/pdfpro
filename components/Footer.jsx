      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-white/25 leading-relaxed mb-5">
              Professional PDF tools. No accounts. No uploads for local processing. Zero data retained.
            </p>
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-gold-400/50" />
              <span className="font-mono text-xs text-gold-400/50">Zero files stored — ever</span>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-1.5">
              {tools.map(t => (
                <Link key={t.href} href={t.href}
                  className="font-mono text-xs text-white/25 hover:text-white/60 transition-colors py-0.5">
                  {t.label}
                </Link>
              ))}
            </div>
          ...
          {/* Legal & Info */}
          <div>
            <p className="label-gold mb-4">Company</p>
            <div className="space-y-2">
              {[
                ...
                <Link key={l.href} href={l.href}
                  className="block font-mono text-xs text-white/25 hover:text-white/60 transition-colors py-0.5">
                  {l.label}
                </Link>
              ))}
            </div>
        ...
        {/* Bottom */}
        <div className="h-px mb-6" style={{background:'linear-gradient(90deg, rgba(201,168,76,0.15), transparent)'}} />
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          ...
          <p className="font-mono text-xs text-white/15">
            ...
            <Link href="/terms" className="text-gold/40 hover:text-gold/70 transition-colors">Terms of Service</Link>
            {' '}and{' '}
            <Link href="/privacy" className="text-gold/40 hover:text-gold/70 transition-colors">Privacy Policy</Link>.
          </p>
        </div>
      </div>
    </footer>
