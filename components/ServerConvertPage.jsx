    ...
    <ToolLayout title={title} icon={icon} description={description} isServer>
      {!file ? (
        ...
        <div className="space-y-4">
          ...
          {serverWarning && (
            <div className="glass-card p-4 border-amber-500/20" style={{borderColor:'rgba(251,146,60,0.2)', background:'rgba(251,146,60,0.05)'}}>
          ...
          {done ? (
            <div className="success-card">
              ...
              <p className="font-heading font-semibold text-emerald-300 text-lg mb-1">Converted successfully!</p>
              <p className="text-slate-400 text-sm mb-4">Your {outputLabel} has been downloaded.</p>
              <button onClick={() => { setFile(null); setDone(false); }} className="btn-glass text-sm">Convert another file</button>
            </div>
          ) : (
